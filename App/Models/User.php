...
class User extends \Core\Model
{
    ...
    public function save(){
        ...
        if (empty($this->errors)) {
            ...
            //password_hash bez $this ponieważ tworzymy zmienną na początku metody. Obiekt nie będzie miał atrybutu password_hash tylko password.
     ...
     * 
     * @return boolean True if email is in DB
     */
    public static function emailExists($email, $ignore_id = null){
        //chcemy wartość bool, funkcja findByEmail zwraca metodę fetch, która zwraca fałsz gdy nie znajdzie rekordu w bazie lub obiekt gdy znajdzie. Gdy więc będzie obiekt !== false wyjdzie true, gdy zaś false !== false to fałsz.
    ...
    public static function findByEmail($email)
    {
       ...
       // dostajemy obiekt klasy User razem z namespace, bez poniższej lini otrzymamy zwykły array.
        $stmt->setFetchMode(PDO::FETCH_CLASS, get_called_class());
        $stmt->execute();
        //fetch jeśli znajdzie match w bazie zwraca obiekt, jeśli nie to false.
        return $stmt->fetch();
    }
    ...
    protected function startPasswordReset(){
        $token = new Token();
        $hashed_token = $token->getHash();
        //przypisanie atrybutu do obiektu
     ...
     * @param string $oken Password reset token sent to user
    ...
    public function resetPassword($password){
        ...
        //sprawdza cyz zmienna jest pusta. false jeśli istnieje i nie jest pusta
     ...
     * @return void
     */
    public function addDefaultIncomeCategories(){
        ...
        $db = static::getDB();
        $stmt1 = $db->prepare($incomeCategoryQuery);
        $stmt1->execute();
        $incomeCategories = $stmt1->fetchAll(PDO::FETCH_ASSOC);
       
        
        for ($i = 0; $i < count($incomeCategories); $i++) {
            $category = $incomeCategories[$i]['name'];
            $insertIncomeCategoryQuery = "INSERT INTO incomes_category_assigned_to_users (user_id, name) VALUES (?,?)";
            $stmt2 = $db->prepare($insertIncomeCategoryQuery);
            $stmt2->execute(array($registeredUserId, $category));
        }
    }
    ...
    public function addDefaultExpenseCategories(){
        ...
        $db = static::getDB();
        $stmt1 = $db->prepare($expenseCategoryQuery);
        $stmt1->execute();
        $expenseCategories = $stmt1->fetchAll(PDO::FETCH_ASSOC);
       
        
        for ($i = 0; $i < count($expenseCategories); $i++) {
            $category = $expenseCategories[$i]['name'];
            $insertExpenseCategoryQuery = "INSERT INTO expenses_category_assigned_to_users (user_id, name) VALUES (?,?)";
            $stmt2 = $db->prepare($insertExpenseCategoryQuery);
            $stmt2->execute(array($registeredUserId, $category));
        }
    }
